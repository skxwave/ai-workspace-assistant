...
class ChatMessageRepository:
    ...
    async def save(self, *, owner_id: uuid.UUID, role: MessageRole, content: str) -> ChatMessage:
    ...
    async def delete_for_owner(self, *, owner_id: uuid.UUID) -> None:
        await self.session.execute(delete(ChatMessage).where(ChatMessage.owner_id == owner_id))
        await self.session.commit()
